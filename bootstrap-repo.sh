...
#
#   ./bootstrap-repo.sh                       # -> rajendrakalla-cpu/nitya-panchang
#   ./bootstrap-repo.sh my-repo-name
#
...
if command -v gh >/dev/null; then
  ...
  curl -fsSL -X POST -H "Authorization: Bearer $GITHUB_TOKEN" \
       ...
       -d "{\"name\":\"$REPO\",\"description\":\"$DESC\",\"private\":false}" >/dev/null || true
  git remote get-url origin >/dev/null 2>&1 \
